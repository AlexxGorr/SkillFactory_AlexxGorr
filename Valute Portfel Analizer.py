...
def show_percent(name, percent, width = 40):
    fill = '#'
    empty = '-'
    bars = round(percent * width)

    progress = fill * bars + empty * (width - bars)
    print(f'{name:8} - {round(percent * 100)}% |{progress}|')

#show_percent('RUB', 0.7)
#show_percent('BITCOIN', 0.3)
#show_percent('USD', 0.5)
...
def loop():
    ...
    while True:
        ...
        if mode == 1:
            ...
            amount = float(input('  Введите кол-во валюты: '))
            data[week].append({
                'name': name,
                'cost': cost,
                'amount': amount
            ...
            for i, curr in enumerate(data[week]):
                ...
                if num == None:
                    ...
                    amount = input(f'  Введите кол-во валюты ({curr_amount}): ')
                    if amount:
